...
pub struct BootContext {
    /// 引导服务表 (ExitBootServices 前可用)
    pub boot_services: *const BootServices,
    /// 帧缓冲 (用于 GOP 渲染)
    pub framebuffer: Option<gfx::FrameBuffer>,
    /// 主题配置
    pub theme: config::ThemeConfig,
    /// 引导条目管理器
    pub entries: config::GenerationManager,
    /// 签名验证器
    pub verifier: security::SignatureVerifier,
    /// TPM 测量器
    pub tpm: security::TpmMeasurer,
    /// 是否显示启动画面
...
impl BootContext {
    pub fn new(boot_services: &BootServices) -> Self {
        Self {
            ...
            entries: config::GenerationManager::new(),
            verifier: security::SignatureVerifier::new(true),
            tpm: security::TpmMeasurer::new(),
            show_splash: true,
