...
- shell 实测：`ln -s` → `ls -l` 显示 `lrwxrwxrwx ... links=1 size=00000007`（07 = 剥掉
  ...
  内容 → `rm` 只摘链接、目标仍在。
- 全量回归 `SELFTEST DONE 1 次 / FAILED+PANIC 0 次`（覆盖 FS-1..FS-19）。
- `make clippy` exit 0（三 crate 0 warning）。

#### M5c 未覆盖（后续切片）

- **跨文件系统的软链接**：绝对目标落在别的挂载点时原样存下，服务端解析不到 → 悬空。
  真要让链接跨 FS 生效，需要在 VFS 层做「重解析 + 二次路由」，是独立的一块机制。
- **`readlink`**：没有读回目标字符串的命令或接口，`ls -l` 只显示类型 `l` 与目标长度
  （`size`），不显示 `link -> target`。因此用户无法从界面确认一个链接具体指向哪里。
- **`lstat`**：`stat` 一律跟随，悬空链接根本 stat 不到（Unix 下 `stat` 同样 ENOENT，
  但那里有 `lstat` 可以看清它是链接）。目前只有 `readdir` 能看出「这是一个链接」。
