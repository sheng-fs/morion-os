...
pub extern "C" fn _start() -> ! {
    ...
    if info.magic != 0x4D4F5249 {
        // Boot Info 无效 — 填满红色屏幕
        unsafe {
            FB.fill_rect(0, 0, FB.width, FB.height, 0x0000FF);
        }
    ...
    // 深色背景
    unsafe {
        FB.fill_rect(0, 0, FB.width, FB.height, 0x08102A);
    }

    // 顶部横幅
    unsafe {
        FB.fill_rect(0, 0, FB.width, 80, 0x1A3A6A);
    }
    ...
    y = info.fb_height - 40;
    unsafe {
        FB.fill_rect(0, y, FB.width, 2, 0x1A3A6A);
    }
    draw_text(
        32,
        y + 10,
        "All checks passed. Kernel is ready for development.",
        gray,
    );
